...
program
    ...
    .option('-t, --translator <translator>', 'Specify which translator to use', null)
    .option('-o, --obj-type <obj-type>', 'If using ObjectsTranslator, specify which type of object is being translated', null)
    ...
    .action((inputFile, outputFile, options) => {
        if (options.list) {
            ...
            console.table(recognizedFileMappings.map((entry) => ({
                Name: entry.name,
                Translator: entry.translator.name,
        ...
        console.info(chalk.white.bold('  Translator:'), `${fileMapper.translator.name}`, !options.translator ? chalk.gray('(Auto-detected based on file name)') : '')
